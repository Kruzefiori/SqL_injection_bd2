...
def create_generic_router(
    service, schema: Type[ModelType], class_name: str
) -> APIRouter:
    ...
    @router.get(f"/{class_name}/{{id}}", description=f"Get a {class_name} by ID")
    ...
    @router.get(f"/{class_name}", description=f"Get all {class_name}")
    ...
    @router.put(f"/{class_name}", description=f"Update a {class_name}")
