...
async def show_orders(request: Request, product_id: int, page: int = 1, per_page: int = 5):
    try:
        ...
        if not orders_data and not metadata.get('message'):
            metadata['message'] = f"Nenhum pedido encontrado para o produto ID {product_id}"
